...
namespace SmartTheme.Core.Extensions
{
    public static class RequestExtensions
    {
        ...
        public static byte[] GetClientIPAddress(this HttpRequest request)
        {
            var ipAddress = request.HttpContext?.Connection?.RemoteIpAddress ?? null;

            if (ipAddress == null)
            {
                Microsoft.Extensions.Primitives.StringValues ipStrVal;
                if (request.Headers.TryGetValue("X-Forwarded-For", out ipStrVal))
                {
                    System.Net.IPAddress netIPAddress;
                    if (System.Net.IPAddress.TryParse(ipStrVal, out netIPAddress) || TryParseIPAddress(ipStrVal, out netIPAddress))
                    {
        ...
        private static bool TryParseIPAddress(string ip, out System.Net.IPAddress ipAddress)
        {
            if (ip.Contains(".") && ip.Contains(":"))
            {
                string ipAdd = ip.Substring(0, ip.IndexOf(':'));
                return System.Net.IPAddress.TryParse(ipAdd, out ipAddress);
            }
            else
            {
                ipAddress = null;
